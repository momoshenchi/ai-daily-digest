...
const HIGH_SCORE_THRESHOLD = 21; // articles scoring >= this (7+7+7) are considered "top quality"
...
function computeFeedScores(articles: Array<{ sourceName: string; sourceUrl: string; score: number }>): FeedScore[] {
  const feedMap = new Map<string, { url: string; scores: number[] }>();

  for (const a of articles) {
    const entry = feedMap.get(a.sourceName) || { url: a.sourceUrl, scores: [] };
    entry.scores.push(a.score);
    feedMap.set(a.sourceName, entry);
  }

  return Array.from(feedMap.entries()).map(([name, { url, scores }]) => {
    const articleCount = scores.length;
    const avgScore = scores.reduce((s, v) => s + v, 0) / articleCount;
    const topArticleCount = scores.filter(s => s >= HIGH_SCORE_THRESHOLD).length;
    // Weighted score: reward sources that consistently produce quality content at volume
    const feedScore = avgScore * Math.log2(articleCount + 1);
    return { name, url, articleCount, topArticleCount, avgScore, feedScore };
  }).sort((a, b) => b.feedScore - a.feedScore);
}

function generateFeedScoreSection(feedScores: FeedScore[]): string {
  if (feedScores.length === 0) return '';

  const top = feedScores.slice(0, 10);
  let section = `## 🏅 信源质量排行\n\n`;
  section += `> 综合评分 = 平均得分 × log₂(文章数 + 1)，越高代表该博客稳定输出高质量内容\n\n`;
  section += `| 排名 | 信源 | 文章数 | 优质文章 | 平均分 | 综合评分 |\n`;
  section += `|:---:|:---|:---:|:---:|:---:|:---:|\n`;

  for (let i = 0; i < top.length; i++) {
    const f = top[i]!;
    const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i + 1}`;
    section += `| ${medal} | [${f.name}](${f.url}) | ${f.articleCount} | ${f.topArticleCount} | ${f.avgScore.toFixed(1)} | ${f.feedScore.toFixed(1)} |\n`;
  }
...
function parseArticlesFromMarkdownContent(content: string, sourceLabel: string): Article[] {
  const articles: Article[] = [];
  const lines = content.split('\n');

  // Matches article link lines generated by generateDigestReport, e.g.:
  //   [Title](https://url) — **Source** · 2 小时前 · ⭐ 25/30
  // Capture groups: 1=title, 2=url, 3=sourceName (strips optional ** bold markers)
  const linkPattern = /\[([^\]]+)\]\((https?:\/\/[^)]+)\)\s*[—–-]{1,2}\s*\*?\*?([^·*\n]+?)\*?\*?\s*·/;
  ...
  const flush = () => {
    if (currentTitle && currentLink) {
      articles.push({
        title: currentTitle,
        link: currentLink,
        // pubDate is set to current time; original dates are not preserved in
        // the markdown format and are not needed for re-scoring purposes.
        pubDate: new Date(),
        description: currentSummary || currentTitle,
        sourceName: currentSource || sourceLabel,
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      currentSource = match[3]!.trim();
    } else if (line.startsWith('> ') && currentLink) {
      const summaryText = line.slice(2).trim();
      if (summaryText) {
        currentSummary = currentSummary ? `${currentSummary} ${summaryText}` : summaryText;
      }
    }
  }
...
async function loadArticlesFromFiles(filePaths: string[]): Promise<Article[]> {
  const allArticles: Article[] = [];
  ...
  for (const filePath of filePaths) {
    try {
      ...
      const articles = parseArticlesFromMarkdownContent(content, fileName);
      console.log(`[digest] Parsed ${articles.length} articles from ${filePath}`);
      allArticles.push(...articles);
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      console.warn(`[digest] Failed to read ${filePath}: ${msg}`);
    }
  }
...
function generateDigestReport(articles: ScoredArticle[], highlights: string, feedScores: FeedScore[], stats: {
  totalFeeds: number;
  successFeeds: number;
  ...
  hours: number;
  lang: string;
  sourceMode?: 'rss' | 'files';
}): string {
  const now = new Date();
  const dateStr = now.toISOString().split('T')[0];
  
  let report = `# 📰 AI 博客每日精选 — ${dateStr}\n\n`;
  if (stats.sourceMode === 'files') {
    report += `> 来自 ${stats.totalFeeds} 个 Markdown 文件，共 ${stats.totalArticles} 篇文章，AI 精选 Top ${articles.length}\n\n`;
  } else {
  ...
  if (stats.sourceMode === 'files') {
    report += `| 输入文件 | 解析文章 | 精选 |\n`;
    report += `|:---:|:---:|:---:|\n`;
    report += `| ${stats.totalFeeds} 个 | ${stats.totalArticles} 篇 | **${articles.length} 篇** |\n\n`;
  } else {
  ...
  // ── Feed Score Leaderboard ──
  const feedScoreSection = generateFeedScoreSection(feedScores);
  if (feedScoreSection) {
    report += feedScoreSection;
    report += `---\n\n`;
  }
...
Options:
  --hours <n>          Time range in hours for RSS mode (default: 48)
  ...
  --input-file <path>  Read articles from a previously generated Markdown digest
                       file instead of fetching RSS feeds. Can be repeated to
                       process multiple files at once. Time filtering is skipped
                       in this mode.
  --help               Show this help
...
Examples:
  ...
  bun scripts/digest.ts --input-file digest-20260401.md --input-file digest-20260402.md --top-n 20
`);
  process.exit(0);
}
...
async function main(): Promise<void> {
  ...
  const fileMode = inputFiles.length > 0;
  ...
  if (fileMode) {
    console.log(`[digest] Mode: Markdown file input (${inputFiles.length} file(s))`);
  } else {
  ...
  console.log(`[digest] Loading prompt templates...`);
  ...
  let statsSourceMode: 'rss' | 'files';
  let statsTotalFeeds: number;
  let statsSuccessFeeds: number;

  if (fileMode) {
    // ── File Mode: load articles from markdown files ──
    console.log(`[digest] Step 1/5: Loading articles from ${inputFiles.length} Markdown file(s)...`);
    allArticles = await loadArticlesFromFiles(inputFiles);

    if (allArticles.length === 0) {
      console.error('[digest] Error: No articles parsed from the provided files. Check file paths and format.');
      process.exit(1);
    }

    console.log(`[digest] Step 2/5: Skipping time filter (file mode) — ${allArticles.length} articles available`);
    recentArticles = allArticles;
    statsSourceMode = 'files';
    statsTotalFeeds = inputFiles.length;
    statsSuccessFeeds = inputFiles.length;
  } else {
    // ── RSS Mode: fetch live feeds ──
    console.log(`[digest] Loading feeds config...`);
    const RSS_FEEDS = await loadFeeds();

    console.log(`[digest] Step 1/5: Fetching ${RSS_FEEDS.length} RSS feeds...`);
    allArticles = await fetchAllFeeds(RSS_FEEDS);
    ...
    statsSourceMode = 'rss';
    statsTotalFeeds = RSS_FEEDS.length;
    statsSuccessFeeds = new Set(allArticles.map(a => a.sourceName)).size;
  }
  ...
  // Compute feed scores from all scored articles (not just top N)
  const feedScores = computeFeedScores(
    scoredArticles.map(a => ({ sourceName: a.sourceName, sourceUrl: a.sourceUrl, score: a.totalScore }))
  );

  if (feedScores.length > 0) {
    console.log(`[digest] Feed score top-3: ${feedScores.slice(0, 3).map(f => `${f.name}(${f.feedScore.toFixed(1)})`).join(', ')}`);
  }
  
  const report = generateDigestReport(finalArticles, highlights, feedScores, {
    totalFeeds: statsTotalFeeds,
    successFeeds: statsSuccessFeeds,
    totalArticles: allArticles.length,
    filteredArticles: recentArticles.length,
    hours,
    lang,
    sourceMode: statsSourceMode,
  });
  ...
  if (fileMode) {
    console.log(`[digest] 📊 Stats: ${inputFiles.length} files → ${allArticles.length} articles → ${finalArticles.length} selected`);
  } else {
    console.log(`[digest] 📊 Stats: ${statsSuccessFeeds} sources → ${allArticles.length} articles → ${recentArticles.length} recent → ${finalArticles.length} selected`);
  }
